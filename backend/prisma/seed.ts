...
async function main() {
  ...
  await prisma.notification.deleteMany() // 清理通知
  await prisma.post.deleteMany()
  await prisma.tag.deleteMany()
  await prisma.user.deleteMany()
  await prisma.foodShowcase.deleteMany() // 新增：清理 FoodShowcase 数据
  console.log('Old data cleaned.')

  // 创建标签
  const tagTech = await prisma.tag.create({ data: { name: '技术' } })
  const tagLife = await prisma.tag.create({ data: { name: '生活' } })
  const tagTravel = await prisma.tag.create({ data: { name: '旅行' } })
  ...
  const adminHashedPassword = await bcrypt.hash(adminPassword, 10); // 10 salt rounds
  ...
  await prisma.foodShowcase.createMany({
    data: foodImages
  });
