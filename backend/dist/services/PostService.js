        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (data.tagNames && data.tagNames.length > 0) {
                ...
                for (const tagName of data.tagNames) {
                    // 1. 创建或获取标签
                    const tag = yield db_1.default.postTag.upsert({
                        where: { name: tagName },
                        update: {}, // 如果存在，不更新任何内容
                        create: { name: tagName, isFixed: false }
                    });
                    // 2. 创建帖子与标签的关联
                    yield db_1.default.postTags.create({
                        data: {
                            A: post.id, // 帖子ID
                            B: tag.id // 标签ID
                        }
                ...
                // 重新获取帖子，包括新创建的标签关联
                const updatedPost = yield db_1.default.post.findUnique({
                    where: { id: post.id },
                    select: {
                ...
                return this.processPostResult(updatedPost);
            }
    ...
    static getAllPosts(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (tags && tags.length > 0) {
                // 获取标签ID
                const tagIds = yield db_1.default.postTag.findMany({
                    where: {
                        name: {
                            in: tags,
                            mode: 'insensitive'
            ...
            const selectClause = Object.assign({ id: true, title: true, content: true, imageUrl: true, createdAt: true, updatedAt: true, authorId: true, isShowcase: true, author: { select: { id: true, name: true, avatarUrl: true } }, PostTags: { select: { A: true, B: true } }, _count: { select: { likes: true, comments: true, favoritedBy: true } } }, (currentUserId && {
    ...
    static updatePost(postId, data, userId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (data.tagNames !== undefined) {
                // 步骤1：删除所有现有标签关联
                yield db_1.default.postTags.deleteMany({
                    where: { A: postId }
                });
                // 步骤2：为每个标签创建新的关联
                for (const tagName of data.tagNames) {
